...
There are 30463 rows and 15 columns in this dataset.

**3. What are the data types of the variables in this dataset?**

The variables species_id, sex, day_of_week, plot_type, genus, species, and taxa are all character types.
The variables record_id, month, day, year, plot_id, hindfoot_length, and weight are double type.
The variable date has a date type.
...
ggplot(data = surveys,
  ...
  labs(title = "Effect of penguin weight on hind foot length by species",
...
Based on our one-way ANOVA model results, we have significant evidence (p = 0, F = 19916) to suggest that at least one rodent species has a different population mean weight.
